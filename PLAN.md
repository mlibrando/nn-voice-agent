...
### Day 5 — Fri Jul 3 · Prompt-for-voice + CX + KNOW-1 + SAFE-3 + TOOL-END + coupled auth-greeting fix — **DONE**

**Landed:**
- **SYSTEM_MESSAGE replaced** (Day-1 placeholder → structured persona). Sections: persona/voice-format, identity verification (updated for open greeting), CX cadence (CX-1..6 encoded), retention micro-sequence (RETN-1/RETN-2), product-knowledge block (KNOW-1) for the 5 seeded SKUs sourced from labels + AIA style (never AIA literal values), health-advice guardrail (SAFE-3) with **broad SAFE-1 trigger** — any wellness/health-adjacent phrasing routes to cancel-with-`reason=other` + medical follow-up + skip retention, billing/refund mechanics (EXPL-1), CX-7 abuse ladder, Realtime-emotion-tag caveat (Risk #9). → CONV-4, CX-1/2/3/4/5/6, RETN-1, RETN-2, KNOW-1, SAFE-3, SAFE-1, EXPL-1, DATA-1
- **Truthfulness gate (CX-5)** — SYSTEM_MESSAGE explicitly forbids "let me talk to my manager" / "I checked with my supervisor" / "as a VIP customer" phrasing. AIA conv 03's fictional-manager framing is replaced by honest concession framing ("Here's the best I can do — a 20% lifetime discount…"), keeping the real outcome without the invented authority. → Risk #11
- **TOOL-END wired.** New `end_call(reason)` tool. Handler posts to Twilio REST API (`Calls/{CallSid}.json?Status=completed`) with basic-auth via `TWILIO_ACCOUNT_SID` + `TWILIO_AUTH_TOKEN` env. Auto-creates a high-risk `create_escalation` server-side before hanging up (audit trail). Prompt-gated (3-step ladder in SYSTEM_MESSAGE), `abuse_strikes` counter incremented on every attempt for observability. In `_PRE_AUTH_TOOLS` — abuse can happen pre-verify. → TOOL-END, CX-7, Risk #12
- **Register-shift dispatch** in `bridge.py`. After certain tool categories (SAFE-1 cancel with `reason="other"`, `end_call`), the follow-up `response.create` includes a per-response `instructions` string nudging warmer/slower prosody. Risk #9 workaround (Realtime has no emotion tags). → CX-2, Risk #9
- **Coupled auth-greeting fix (bundled, non-splittable) landed:**
  - **(1) Open Tier-0 greeting.** `bridge.py` greeting instructions no longer name the caller. Instructions are identical on Tier-0 hit and miss — "who do I have the pleasure of speaking with?" or "am I speaking with the account holder?". The Tier-0 hit context still carries the located `first_name` in the system-context message so the model knows who to verify against. Coupling comment in `bridge.py` points at `handlers._is_affirmative`. → CX-6 (confirm-before-name), F6 test target
  - **(2) `_is_affirmative` tightened.** Old substring-match `_AFFIRMATIVE_WORDS` set (which contained "this is" and "speaking") replaced with `_AFFIRMATIVE_TOKENS` + `_STOP_TOKENS` + `_MULTI_WORD_AFFIRMATIVES`. New logic: whole-word first-name check, multi-word affirmative phrases, or every token in the affirmative-or-stop union. "This is Bob" against `first_name="Margaret"` now fails; "This is Margaret", "Margaret", "yes", "yeah, I am" all pass. Coupling comment in `handlers.py` points at `bridge.py`. **AUTH-17** is the regression guard. → coupled with (1); shipping either without the other opens a false-positive Tier-0 verification for anyone claiming to be the holder
- **5 new AUTH tests + END-1** in `scripts/test_tools.py` — AUTH-17 (impostor "This is Bob"), AUTH-18 ("This is Margaret" verifies), AUTH-19 ("yes"), AUTH-20 (bare-name open-greeting answer), AUTH-21 ("Yeah, Bob"), END-1 (end_call handler shape, `_PRE_AUTH_TOOLS` inclusion, graceful missing-creds path, dispatch-gate). All green.
- **TESTING.md Section G** — 8 CX/persona live-call tests. G3 is the live-call regression test for AUTH-17. G5/G5b/G5c triangulate the SAFE-1 broad-trigger boundary (G5 = clear symptom, G5b = ambiguous phrasing routes to SAFE-1, G5c = pure preference/cost stays on retention). F6 updated to reflect landed status.
- **DECISIONS.md** — 3 new 🚧 DRAFT entries: "The truthfulness gate — no fake manager", "SAFE-1 broad-trigger — err on the side of the guardrail", "`end_call` — prompt-gated for Day 5, blast-radius reasoned". Greeting-phrasing paragraph in draft #1 updated from "open follow-up" to "landed Day 5".

**Deferred (per scope):**
- Disambiguation behavior for `cust_004` — already covered by Day-4 dispatch (CONV-3 disambiguation is bridge/model behavior; F9 exercises it live). No additional prompt work needed.
- Filler on tool dispatch (Day 6 — `TODO Day 6` marker in `bridge.py` still in place).
- P1 tools TOOL-4/7/9/12/13 (Day 6).
- Handler-side `abuse_strikes` gate on `end_call` (Day 6+ hardening if G7 shows drift).
